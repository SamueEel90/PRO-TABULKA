								...
								+ visibleMonthEntries.map(function(entry) {
									...
									switch (row.type) {
										...
										case 'delta':
											return '<td class="' + focusedCellClass + ' ' + getDeltaCellClass(value) + '"><span class="delta-value">' + formatSignedMetric(value, section.format) + '</span></td>';
										case 'plan-delta': {
											var planVal = itemPlanRow.values[entry.index];
											var pct = planVal ? Math.round(Math.abs(value / planVal) * 1000) / 10 : null;
											var pctStr = pct !== null ? '<span class="delta-pct">' + (value >= 0 ? '+' : '') + pct + '%</span>' : '';
											return '<td class="' + focusedCellClass + ' ' + getDeltaCellClass(value) + '"><span class="delta-value">' + formatSignedMetric(value, section.format) + pctStr + '</span></td>';
										}
										case 'forecast':
			...
			function renderMetricYearTotalCell(row, format, planTotal, sectionOrMetric) {
				...
				if (row.type === 'delta' || row.type === 'plan-delta') {
				return '<td class="year-total-cell ' + getDeltaCellClass(row.total) + '"><span class="delta-value">' + formatSignedMetric(row.total, format) + '</span></td>';
			}
			return '<td class="year-total-cell">' + formatMetric(row.total, format) + '</td>';
			}
