  ...
  try {
    ...
    switch (String(method || '')) {
      ...
      case 'getDashboardData':
        return NextResponse.json({ ok: true, result: await getDashboardDataFromSql(resolveLogin(String(params[0] || '')), String(params[1] || 'ALL')) });
      case 'getSummaryData':
        return NextResponse.json({ ok: true, result: await getSummaryDataFromSql(resolveLogin(String(params[0] || ''))) });
      case 'getWeeklyVodOverrides':
        return NextResponse.json({ ok: true, result: await getWeeklyVodOverridesFromSql(resolveLogin(String(params[0] || '')), String(params[1] || 'ALL'), String(params[2] || '')) });
      case 'getScopeNotes':
        return NextResponse.json({ ok: true, result: await getScopeNotesFromSql(resolveLogin(String(params[0] || '')), String(params[1] || 'ALL'), String(params[2] || 'scope'), String(params[3] || '')) });
