...
const DEFAULT_SHEET_NAME = 'PLANGJ2026';
...
function printUsage() {
  console.log([
    'Usage:',
    '  npm run import:plan -- --file <path-to-plan-file> [--sheet PLANGJ2026] [--uploaded-by <name>]',
    '',
    'Examples:',
    '  npm run import:plan -- --file ..\\PLANGJ2026.xlsx',
    '  npm run import:plan -- --file ..\\plan.csv --uploaded-by samuel@firma.sk',
  ].join('\n'));
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (token === '--sheet') {
      options.sheet = String(argv[index + 1] || '');
      index += 1;
      continue;
    }

    if (token === '--uploaded-by') {
      options.uploadedBy = String(argv[index + 1] || '');
      index += 1;
      continue;
    }
  ...
  if (!options.file && positional[0]) {
    options.file = String(positional[0] || '');
  }

  if (!options.uploadedBy && positional[1]) {
    options.uploadedBy = String(positional[1] || '');
  }
...
async function upsertReferenceData(prisma, values) {
...
async function createMonthlyValues(prisma, batchId, values) {
...
async function main() {
  ...
  try {
    ...
    if (!parsedImport.values.length) {
      throw new Error('V importovanom PLANGJ2026 súbore sa nenašli žiadne hodnoty.');
    }

    await upsertReferenceData(prisma, parsedImport.values);

    const importedStoreIds = Array.from(new Set(parsedImport.values.map((value) => value.storeId)));
    const importedMonthIds = Array.from(new Set(parsedImport.values.map((value) => value.monthId)));

    await prisma.monthlyValue.deleteMany({
      where: {
        source: 'PLAN',
        storeId: { in: importedStoreIds },
        monthId: { in: importedMonthIds },
      },
    });
    ...
    const batch = await prisma.importBatch.create({
      data: {
        ...
        rowCount: parsedImport.values.length,
      },
    });

    await createMonthlyValues(prisma, batch.id, parsedImport.values);

    console.log(`Imported PLAN data from ${path.basename(filePath)}.`);
    console.log(`Sheet: ${parsedImport.sheetName}`);
    console.log(`Rows: ${parsedImport.values.length}`);
    console.log(`Stores: ${importedStoreIds.length}`);
    console.log(`Months: ${importedMonthIds.length}`);
